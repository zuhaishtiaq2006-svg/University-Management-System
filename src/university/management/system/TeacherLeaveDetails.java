...
public class TeacherLeaveDetails extends JFrame implements ActionListener {
    ...
    public TeacherLeaveDetails() {
        ...
        heading.setBounds(40, 20, 250, 25);
        heading.setFont(new Font("serif", Font.BOLD, 20));
        heading.setForeground(Color.WHITE);
        background.add(heading);

        cEmpId = new Choice();
        cEmpId.setBounds(300, 25, 200, 30);
        ...
        try {
            Conn c = new Conn();
            ResultSet rs = c.s.executeQuery("select * from teacherrleave");
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == search) {
            String query = "select * from teacherrleave where empId = '" + cEmpId.getSelectedItem() + "'";
            try {
                Conn c = new Conn();
